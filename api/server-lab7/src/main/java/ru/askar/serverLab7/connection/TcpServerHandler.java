package ru.askar.serverLab7.connection;
...
import ru.askar.serverLab7.CollectionCommandExecutor;
...
public class TcpServerHandler implements ServerHandler {
    ...
    private void processCommand(SocketChannel channel, CommandToExecute command) {
        requestProcessorExecutor.submit(() -> {
            try {
                CommandResponse response = collectionCommandExecutor.execute(command.name(), command.args(), command.object(), command.credentials());
